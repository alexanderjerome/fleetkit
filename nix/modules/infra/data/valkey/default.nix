...
#
# Usage in hosts.nix:
#   infra.data.valkey.enable = true;                       # sidecar form
#   infra.data.valkey.bindAddress = "192.0.2.16";          # dedicated form
#   infra.data.valkey.allowedSubnets = [ "192.0.2.0/24" ];
{ config, lib, pkgs, ... }:
let
  inherit (lib) mkEnableOption mkOption mkIf types;
  cfg = config.infra.data.valkey;
