...
let
  ...
  cfg = config.infra.data.postgresql.backup;
  ...
  # infra.data.postgresql, so anchoring on the upstream NixOS option lets
...
in
{
  options.infra.data.postgresql.backup = {
    ...
    s3 = {
      ...
      bucket = mkOption {
        type = types.str;
        default = "pg-backups";
        description = "Garage bucket. Provisioned by infra.data.s3 on the s3 host.";
      };
