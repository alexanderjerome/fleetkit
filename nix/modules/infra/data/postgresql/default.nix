...
let
  inherit (lib) mkEnableOption mkOption mkIf types concatStringsSep
    mapAttrsToList;
  cfg = config.infra.data.postgresql;
...
in
{
  imports = [
    ...
    # infra.data.postgresql.backup.enable; inert until enabled.
    ./pgbackrest.nix
  ...
  config = mkIf cfg.enable {
    ...
    infra.data.postgresql.pgwebAccess.databases = builtins.attrNames cfg.databases;
    infra.data.postgresql.pgwebAccess.port = lib.mkDefault cfg.tcpPort;
  };
}
