...
let
  ...
  cfg = config.infra.pki.acmeDns;
in
{
  options.infra.pki.acmeDns = {
    ...
    assertions = [
      {
        ...
        message = "infra.pki.acmeDns.enable is set but domain/nsname/nsadmin are null — set fleet.settings.domain.base (or infra.pki.acmeDns.{domain,nsname,nsadmin} explicitly).";
      }
      {
        assertion = cfg.publicIp != null;
        message = "infra.pki.acmeDns.enable is set but infra.pki.acmeDns.publicIp is null — set fleet.settings.network.wanIp (or infra.pki.acmeDns.publicIp explicitly).";
      }
    ];
