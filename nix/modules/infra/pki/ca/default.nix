...
in
{
  options.infra.pki.ca = {
    ...
    domain = mkOption {
      ...
      description = "FQDN for the CA server. Must be non-null when infra.pki.ca is enabled (asserted).";
    };
  ...
  config = mkIf cfg.enable {
    assertions = [{
      ...
      message = "infra.pki.ca.enable is set but infra.pki.ca.domain is null — set fleet.settings.domain.internal (or infra.pki.ca.domain explicitly).";
    }];
