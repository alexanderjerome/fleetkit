...
let
  ...
  cfg = config.infra.observability.tempo;
in
...
# carries no `protect`.
{
  options.infra.observability.tempo = {
    ...
    otlpGrpcAddress = mkOption {
      ...
      description = ''
        ...
        gateway's own :4317. Must match infra.observability.alloy.gateway.tempoOtlpEndpoint.
      '';
    };
  ...
  config = mkIf cfg.enable {
    services.tempo = {
      ...
      settings = {
        ...
        # Opt-in via infra.observability.tempo.mcpServer.enable on the host.
