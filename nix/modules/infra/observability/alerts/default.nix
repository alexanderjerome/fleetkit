# infra.observability.alerts — fleet alert aggregation (INFRA-106).
#
# Service modules contribute Grafana alert rules close to their own config via
# `infra.observability.alerts.rules`. Each host therefore carries the alert rules for the
...
#
# Per-host opt-out: set `infra.observability.alerts.enable = false` on a host to drop ALL of
...
in
{
  options.infra.observability.alerts = {
    enable = mkOption {
      type = types.bool;
      default = true;
      description = ''
        Whether this host contributes its alert rules to the fleet. When false,
        none of this host's `infra.observability.alerts.rules` are collected/provisioned by the
    ...
    rules = mkOption {
      ...
      description = ''
        ...
        which gathers this across all nodes with `infra.observability.alerts.enable = true`.
      '';
    };
