...
in
{
  # Sub-feature: scheduled end-to-end directory-auth probe
  # (infra.auth.sssd.probe — declared in ./probe.nix).
  imports = [ ./probe.nix ];

  options.infra.auth.sssd = {
    ...
    ldapUri = mkOption {
      ...
      description = "URI of the Authentik LDAP outpost. Must be non-null when infra.auth.sssd is enabled (asserted).";
    };
    ...
    baseDn = mkOption {
      ...
      description = "LDAP base DN for user/group searches. Must be non-null when infra.auth.sssd is enabled (asserted).";
    };
  };
    ...
    assertions = [
      {
        ...
        message = "infra.auth.sssd.enable is set but infra.auth.sssd.ldapUri is null — set fleet.network.ldap.uri (or infra.auth.sssd.ldapUri explicitly).";
      }
      {
        assertion = cfg.baseDn != null;
        message = "infra.auth.sssd.enable is set but infra.auth.sssd.baseDn is null — set fleet.network.ldap.base_dn (or infra.auth.sssd.baseDn explicitly).";
      }
    ];
