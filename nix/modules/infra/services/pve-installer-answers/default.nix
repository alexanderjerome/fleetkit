...
# infra.pve-installer-answers — HTTP answer-file server for PVE/PBS
...
let
  ...
  cfg = config.infra.pve-installer-answers;
  ...
  renderAnswerToml = host:
    let
      ...
      '';
    in ''
      # Generated by infra.pve-installer-answers for ${host.hostname} (${host.productType})
...
in
{
  options.infra.pve-installer-answers = {
