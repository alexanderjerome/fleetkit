...
let
  ...
  # Module-contributed alerts: each node appends to infra.alerts.rules next to
  # its own service config; collect them fleet-wide here via Colmena `nodes`,
  # skipping hosts that set infra.alerts.enable = false. Empty under a plain
  # nixosConfigurations eval (no `nodes`); populated on a Colmena deploy.
  fleetContributedRules = lib.concatMap
    (node: lib.optionals node.config.infra.alerts.enable node.config.infra.alerts.rules)
  ...
  # Consumer-supplied rule directories (infra.grafana-stack.alertRulesDirs)
  ...
  # pre-rendered JSON directories via infra.grafana-stack.dashboardsDirs.
  nixDashboards = {
    "Fleet" = {
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    #   1. Bucket — declared in infra.garage-bootstrap.buckets on the s3
