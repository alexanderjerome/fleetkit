...
# (infra.postgresql, infra.app-db, infra.timescale-db, …). Clients point their
...
#   infra.pgbouncer = { enable = true; databases = [ "app_db" ]; };
#
...
let
  ...
  cfg = config.infra.pgbouncer;
in {
  options.infra.pgbouncer = {
  ...
  config = mkIf cfg.enable {
    assertions = [{
      ...
      message = "infra.pgbouncer.enable is true but infra.pgbouncer.databases is empty — nothing would be poolable.";
    }];
