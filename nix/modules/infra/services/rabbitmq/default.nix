...
#
# Usage in hosts.nix:
#   infra.rabbitmq.enable = true;
#   infra.rabbitmq.appPassword = "...";
{ config, lib, pkgs, ... }:
let
  inherit (lib) mkEnableOption mkOption mkIf mkDefault types;
  cfg = config.infra.rabbitmq;
in
{
  options.infra.rabbitmq = {
  ...
  config = mkIf cfg.enable {
    assertions = [{
      ...
      message = "infra.rabbitmq.enable is set but fleet.settings.domain.internal is null — the rabbitmq.<domain.internal> management vhost needs it.";
    }];
    ...
    infra.caddy.virtualHosts."rabbitmq.${config.fleet.settings.domain.internal}".extraConfig = ''
