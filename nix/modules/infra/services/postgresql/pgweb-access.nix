...
# database via `infra.postgresql.pgwebAccess.databases`.
#
...
# through infra.postgresql (core), infra.timescale-db, infra.app-db
...
let
  ...
  cfg = config.infra.postgresql.pgwebAccess;
...
in
{
  options.infra.postgresql.pgwebAccess = {
    enable = mkOption {
      type = types.bool;
      default = true;
      description = ''
