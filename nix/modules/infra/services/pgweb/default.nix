...
# `infra.postgresql.pgwebAccess` export (see
...
let
  ...
  pgNodes = lib.filterAttrs (name: node:
    ...
    && node.config.infra.postgresql.pgwebAccess.enable
    && node.config.infra.postgresql.pgwebAccess.databases != []
  ) nodes;
