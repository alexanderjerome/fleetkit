...
let
  ...
  cfg = config.infra.network.dhcp;
in
{
  options.infra.network.dhcp = {
    ...
    domain = mkOption {
      ...
      example = "example.lan";
      description = "Domain name advertised to clients. Must be non-null when infra.network.dhcp is enabled (asserted).";
    };
  ...
  config = mkIf cfg.enable {
    assertions = [{
      ...
      message = "infra.network.dhcp.enable is set but infra.network.dhcp.domain is null — set fleet.network.dns_domain (or infra.network.dhcp.domain explicitly).";
    }];
